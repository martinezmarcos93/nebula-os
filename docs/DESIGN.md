...
| **`10-base.sh`** | `apt` de: `xserver-xorg-core xinit x11-xserver-utils bspwm sxhkd picom rofi alacritty dunst feh brightnessctl playerctl network-manager-gnome pavucontrol pipewire pipewire-pulse wireplumber lxpolkit i3lock xss-lock fonts-jetbrains-mono fonts-inter papirus-icon-theme` (el driver NVIDIA **no** se instala aquí: es responsabilidad del post-formateo, fuera de alcance — ver §2.1). Detecta gestor de display activo y aplica el mecanismo de arranque correspondiente (§4). Crea `~/.xinitrc`. Habilita PipeWire de usuario. | `startx`/DM entra a `bspwm` con fondo negro; `Super+Enter` abre Alacritty. |
